@php
    $headerLogoPath = \App\Models\SiteContent::get('header_logo');
    $headerLogoUrl  = $headerLogoPath
        ? asset('storage/' . $headerLogoPath)
...
<header class="z-50 bg-white sticky top-0 w-full shadow-sm">
    <div class="h-20 flex items-center justify-between max-w-7xl mx-auto px-6">
        ...
        <a href="/" class="rounded-2xl">
            <img
                class="w-44 md:w-60 rounded-2xl object-contain max-h-14"
                src="{{ $headerLogoUrl }}"
                alt="Logo">
        </a>
...
<div id="overlay" class="fixed inset-0 bg-black/40 opacity-0 invisible transition-all duration-500 z-40"></div>
...
<div id="sidebar"
    ...
    <div class="p-6 flex justify-between items-center">
        <h2 class="font-semibold text-lg text-[#1E3A8A]">Menu</h2>
        <button id="close-btn">
