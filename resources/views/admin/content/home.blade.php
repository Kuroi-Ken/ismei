<x-admin-layout title="Edit Home Content">
    ...
    <div class="bg-white rounded-2xl shadow p-7 mb-5 w-full">
        <div class="flex items-start gap-3 mb-5">
            ...
            <div>
                ...
                <p class="text-sm text-black/50">
                    Upload up to 3 partner logos shown on the homepage hero. You can label each one and delete them individually.
                </p>
            </div>
        </div>
            ...
            <div class="grid grid-cols-3 gap-4 mb-6">
                @foreach($partnerLogos as $logo)
                    <div class="border border-blue-100 rounded-xl p-4 bg-blue-50/30 flex flex-col gap-3">
                        ...
                        {{-- Edit label --}}
                        <form action="{{ route('admin.content.logos.update-name', $logo->id) }}" method="POST"
                            class="flex gap-2">
                            @csrf
                            @method('PATCH')
                            <input type="text" name="name"
                                value="{{ $logo->name ?? '' }}"
                                placeholder="e.g. SEAMEO"
                            ...
                            <button type="submit"
                                class="px-3 py-1.5 bg-blue-900 text-white text-xs rounded-lg hover:bg-blue-800 transition flex-shrink-0">
                                Save
                            </button>
                        </form>

                        {{-- Delete --}}
                        <form action="{{ route('admin.content.logos.delete', $logo->id) }}" method="POST"
                            onsubmit="return confirm('Delete this logo?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                class="w-full py-1.5 bg-red-50 text-red-600 text-xs rounded-lg hover:bg-red-600 hover:text-white transition flex items-center justify-center gap-1">
        ...
        {{-- Upload form (only if slots available) --}}
        @if($logoCount < 3)
            ...
            <form action="{{ route('admin.content.logos.upload') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="border border-blue-100 rounded-xl p-5 bg-blue-50/40">
                    ...
                    <div id="logo-rows" class="flex flex-col gap-3">
                        <div class="logo-row flex gap-3 items-center">
                            <input type="file" name="logos[]" accept="image/*" required
                                class="flex-1 text-sm text-black/60 border border-blue-200 rounded-xl px-3 py-2 cursor-pointer bg-white
                    ...
                    @if($slotsLeft > 1)
                        <button type="button" id="add-logo-row"
                            class="mt-3 text-xs text-blue-700 hover:text-blue-900 flex items-center gap-1.5 transition">
                    ...
                    btn.addEventListener('click', function () {
                        ...
                        row.innerHTML = `
                            ...
                            <input type="text" name="logo_names[]" placeholder="Label"
                                class="w-44 flex-shrink-0 border border-blue-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300">
                            <button type="button"
                                onclick="this.closest('.logo-row').remove(); rowCount--; if(rowCount < maxRows) document.getElementById('add-logo-row').classList.remove('hidden'); feather.replace();"
